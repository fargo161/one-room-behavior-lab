# Trapstar NPC Encounter v0.1.1

This directory contains the bounded disputed-payment encounter used by the Trapstar capstone browser reference. It demonstrates one NPC, three scripted rounds, three choices per round, persistent encounter-local Trust and Tension, conditional reactions, four moods, three endings, a cinematic setup, and a full restart.
...
Play opens on portable Apartment 305 setup copy, then presents Marcus “Broker” Hill, authored choice-intent labels, and player-safe ending explanations. The player projection omits Trust, Tension, mood labels, round/node language, effects, and threshold rules. `Show Designer View` is off by default and reveals a separate read-only projection of the authoritative encounter state.

Authoritative encounter state includes a bounded ordered `selectedChoiceIds` path. The runtime appends one stable choice ID after current-node validation, includes the full ordered path in deterministic state identity, and clears it on restart. This field is encounter-local path state, not canonical Living Comic History. Ending factors are projection-owned: the projection resolves the selected IDs against portable data and returns the three authored `outcomeInterpretation` strings in order.

## Creating a future bounded encounter

Replace the exported definition in `trapstarPaymentEncounter.ts` with another object accepted by `encounterDefinitionSchema`. Keep stable snake-case IDs, portable setup/profile copy, three ordered rounds with three choices each, authored intent/interpretation metadata, ordered conditional variants, state effects, mood rules, and ordered endings with player-safe outcome leads. No browser callbacks, React components, DOM references, CSS classes, or artwork paths belong in encounter data. Visual mood-to-art mapping stays in the browser renderer.
