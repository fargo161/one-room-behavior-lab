...
describe("trace, performance, validation, and removed shortcuts", () => {
  ...
  it("67 source contains no v0.1 card, buzzer, seed, or truth shortcuts", () => { const files = ["src/core/types.ts", "src/data/scenario.ts", "src/data/propositions.ts", "src/simulation/engine.ts", "app/BehaviorLab.tsx"]; const source = files.map((file) => readFileSync(file, "utf8")).join("\n"); expect(source).not.toMatch(/scenario\.cards|triggerBuzzer|intendedFunctionTags|truthStatusInWorld|\.seed\b|buzzer/i); });
